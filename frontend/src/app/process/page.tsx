...
const RackUnit: React.FC<RackUnitProps> = ({ label, isActive, isComplete, rotation }) => (
  <div className="rack-unit p-4 sm:p-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 sm:gap-0 group relative">
    ...
    <div className="relative w-32 h-16 bg-[#F2E8DC] rounded-t-lg overflow-hidden border-2 border-[#161711] shadow-inner ml-auto mr-12 opacity-90">
      ...
      <div className="absolute bottom-[-2px] left-1/2 w-[2px] h-14 bg-red-600 origin-bottom needle-transition z-10" style={{ transform: `translateX(-50%) rotate(${rotation}deg)` }} />
    ...
    const runProcessing = async () => {
      ...
      try {
        ...
        if (!startResponse.ok) {
          ...
          try {
            const errorBody = (await startResponse.json()) as {
              error?: string;
              code?: string;
          ...
          try {
            ...
            if (!statusRes.ok) {
              if (statusRes.status === 404) {
                throw new Error(
                  "Separation job not found or has expired. Please start a new separation.",
                );
              }

              // For 5xx or other errors, try to surface backend message if available
              let message =
                "Temporary issue contacting the AI engine. Please wait a moment and retry.";
              try {
            ...
            if (
              ...
              throw new Error(
                jobStatus.error || "Separation job failed on the backend.",
              );
            }
          } catch (pollErr) {
    ...
    >
      <div className="min-h-screen bg-background text-foreground p-4 sm:p-8">
        ...
        <div className="bg-metal-dark p-4 sm:p-6 rounded-t-lg border-x border-t border-[#45362C] shadow-2xl relative overflow-hidden">
          {/* Screws */}
          <div className="absolute top-4 left-4 w-3 h-3 rounded-full bg-zinc-700 shadow-inner flex items-center justify-center">
            <div className="w-full h-[1px] bg-black rotate-45" />
          </div>
          <div className="absolute top-4 right-4 w-3 h-3 rounded-full bg-zinc-700 shadow-inner flex items-center justify-center">
            <div className="w-full h-[1px] bg-black rotate-45" />
          </div>
          ...
          <div className="text-center">
            <div className="inline-flex items-center gap-3 mb-2 opacity-80">
              <div
                className={`w-2 h-2 rounded-full ${status.stage === "error" ? "bg-red-500" : "bg-green-500"} led-glow animate-pulse`}
              />
              <span className="font-mono text-xs text-[#A8977A] tracking-[0.2em] uppercase">
        ...
        <div className="bg-[#161711] border-x border-[#45362C]">
          {error ? (
            <div className="p-6 sm:p-12 text-center bg-red-900/20">
              ...
              <button
                onClick={handleRetry}
                className="btn-tactile px-6 py-2 text-[#161711] font-bold rounded-sm"
              >
                RETRY SEQUENCE
              </button>
            </div>
            ...
            <div className="divide-y divide-black">
              {Object.entries(STEM_CONFIG).map(([key, config], index) => {
                ...
                if (isComplete) rotation = 45;
                  ...
                  rotation = -10 + Math.random() * 20 + status.progress / 3; // Simulated movement

                return (
                  <div
                    key={key}
                    className="rack-unit p-4 sm:p-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 sm:gap-0 group relative"
                  >
                    {/* Screw details */}
                    <div className="absolute left-2 top-1/2 -translate-y-1/2 w-2 h-2 rounded-full bg-black/50" />
                    <div className="absolute right-2 top-1/2 -translate-y-1/2 w-2 h-2 rounded-full bg-black/50" />
                    ...
                    <div className="flex items-center gap-4 sm:gap-6 pl-4 sm:pl-6">
                      {/* Channel Label */}
                      <div className="w-20 sm:w-24">
                        <span className="block text-xs text-[#A8977A]/60 font-mono uppercase tracking-widest mb-1">
                          Channel
                        </span>
                        <h3 className="text-lg sm:text-xl font-bold text-[#F2E8DC] uppercase">
                          {config.label}
                        </h3>
                      </div>
                      ...
                      <div className="flex flex-col items-center gap-1">
                        <div
                          className={`w-3 h-3 rounded-full transition-all duration-300 ${isComplete ? "bg-green-500 led-glow" : isActive ? "bg-amber-500 animate-pulse" : "bg-zinc-800"}`}
                        />
                    ...
                    <div className="relative w-32 h-16 bg-[#F2E8DC] rounded-t-lg overflow-hidden border-2 border-[#161711] shadow-inner ml-auto mr-12 opacity-90">
                      ...
                      <span className="absolute top-2 right-2 text-[8px] font-bold text-red-700">
                        PEAK
                      </span>
        ...
        <div className="bg-metal-dark p-6 rounded-b-lg border-x border-b border-[#45362C] shadow-2xl">
          <div className="flex items-center gap-4">
            <span className="font-mono text-xs text-[#A8977A] uppercase w-24">
              Master Out
            </span>
            ...
            <span className="font-mono text-xs text-[#F2E8DC] w-10 text-right">
